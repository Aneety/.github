...
Criar issues `[teste-integracao-api][<responsabilidade>] validar API integrada à camada de dados real do ciclo` após backend verde. Aceite mínimo: API integrada à camada de dados real do ciclo, casos positivos e negativos, isolamento por tenant e evidência do run. Em labels, usar `ciclo:teste-integracao-api`.
